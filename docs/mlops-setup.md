...
```
git init --initial-branch=main_mlops_stack
```
...
Commit the current `README.md` file and other docs to the `main_mlops_stack` branch of the repo, to enable forking the repo:
```

git add README.md docs .gitignore my_mlops_stacks/resources/README.md
git commit -m "Adding project README"

git push upstream main_mlops_stack
```
...
- **`my_mlops_stacks-tests-ci.yml`**:<br>
  - **[CI]** Performs unit and integration tests<br>
    - Triggered on PR to main
- **`my_mlops_stacks-bundle-cicd.yml`**:<br>
  - **[CI]** Performs validation of Databricks resources defined under `my_mlops_stacks/resources`<br>
...
1. Create a new variable group called `my_mlops_stacks variable group` defining the following secret variables, for more details [here](https://learn.microsoft.com/en-us/azure/devops/pipelines/library/variable-groups?view=azure-devops&tabs=classic#create-a-variable-group):
...
1. Define [build validation branch policies](https://learn.microsoft.com/en-us/azure/devops/repos/git/branch-policies?view=azure-devops&tabs=browser#build-validation) for the `main_mlops_stack` branch using the Azure build pipelines created in step 1. This is required so that any PR changes to the `main_mlops_stack` must build successfully before PRs can complete. 
...
After the pull request merges, pull the changes back into your local `main_mlops_stack`
branch:

```
git checkout main_mlops_stack
git pull upstream main_mlops_stack
```

## Create release branch
Create and push a release branch called `release_mlops_stack` off of the `main_mlops_stack` branch of the repository:
```
git checkout -b release_mlops_stack main_mlops_stack
git push upstream release_mlops_stack
git checkout main_mlops_stack
```

Your production jobs (model training, batch inference) will pull ML code against this branch, while your staging jobs will pull ML code against the `main_mlops_stack` branch. Note that the `main_mlops_stack` branch will be the source of truth for ML resource configs and CI/CD workflows.

For future ML code changes, iterate against the `main_mlops_stack` branch and regularly deploy your ML code from staging to production by merging code changes from the `main_mlops_stack` branch into the `release_mlops_stack` branch.

## Deploy ML resources and enable production jobs
Follow the instructions in [my_mlops_stacks/resources/README.md](../my_mlops_stacks/resources/README.md) to deploy ML resources
and production jobs.
...
[ML resource config guide](../my_mlops_stacks/resources/README.md)  to propose, test, and deploy
